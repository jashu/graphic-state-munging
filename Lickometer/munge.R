...
library(data.table)
##For example:
hab = fread('hab.txt') #Habituation session
ret = fread('ret.txt') #Single-trial retrieval session
ext17 = fread('ext17.txt') #Extinction session following a retrieval session
ext18 = fread('ext18.txt') #Normal extinction session
ltm = fread('ltm.txt') #Long-term memory test session
sr = fread('sr.txt') #Spontaneous recovery session
